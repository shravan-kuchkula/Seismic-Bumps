...
The dangers associated with coal mining are myriad -- black lung, flammable gas pockets, rockbursts, and tunnel collapses are all very real dangers that mining companies must consider when attempting to provide safe working conditions for miners.  One class of mining hazard, commonly called 'seismic hazards' are notoriously difficult to protect against, and even more difficult to predict with certainty.  It is, therefore, a well-known problem for machine learning and predictive analytics.  The UCI Machine Learning Repository provides a 'seismic bumps' data set that contains many records of combined categorical and numeric variables that characterize the state of a mine in the Zabrze-Bielszowice coal mine in Poland. 

Our analysis attempts to use logistic regression techniques to predict whether a seismic 'bump' is predictive of a notable seismic hazard, and then compares the results against the state of the art results from other statistical and machine learning techniques.
...
The data were taken from from instruments in the Zabrze-Bielszowice coal mine, in Poland.  There are 2,584 records, with only 170 class = 1 variables, so the data are significantly skewed.  Field descriptions are below, but essentially energy readings and bump counts during one shift (8 hours) are used to predict a 'hazardous' (>10,000J) bump during the next shift.  A class = 1 signifies that a harzardous bump did, indeed, occur in the following shift.  For the sake of reference, a practical example of 10,000 Joules would be the the approximate energy required to lift 10,000 tomatoes 1m above the ground.  Here is an example of the fields in the data set.  
...
The data are mostly numeric with 5 categorical variables (one of which is the outcome variable, class).  However, since the 'nbumps' variables only have a small number of discrete values, they can also be treated as categorical ordinal variables, leaving 12 categorical input variables, 6 numeric variables and one categorical (binary) output variable.  
